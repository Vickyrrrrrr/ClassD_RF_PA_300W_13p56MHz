...
# Since the file starts with (kicad_pcb and ends with ), we want to parse the elements inside it.
# We will find the matching parenthesis of the main (kicad_pcb ...).
# Since it wraps the entire file, we can find the inner blocks.

def get_top_level_blocks(s):
    # Find the starting (kicad_pcb
    match = re.search(r'\(kicad_pcb', s)
    if not match:
        raise ValueError("Not a valid kicad_pcb file")
    
    start_pos = match.end()
    # Find matching closing parenthesis at the very end
    # We will iterate through the file and extract top-level blocks inside kicad_pcb
    blocks = []
    i = start_pos
    while i < len(s):
        # Skip whitespace
        ...
        # If it's a comment or closing paren of kicad_pcb, break/skip
        if s[i] == ')':
            # This should be the last closing paren
            break
        ...
        if s[i] == '(':
            ...
            # Non-block element (like version, generator, etc.)
            start = i
            for j in range(i, len(s)):
...
# 1. +72V5 (Vdd): J1-1 -> C1-1 -> C2-1 -> Q1-2
...
new_elements.extend(path([
    (16.4, 12.0), (16.4, 4.0), (58.0, 4.0), (58.0, 15.0)
], "+72V5"))
...
# Via near C3-1 (16.4, 45.0) to B.Cu
new_elements.append(via(16.4, 43.0, "VCC"))
new_elements.extend(path([
    (16.4, 45.0), (16.4, 43.0)
], "VCC"))
# Route on B.Cu from C3 via to D1-1 (24.92, 8.0)
new_elements.extend(path([
    (16.4, 43.0), (34.0, 43.0), (34.0, 8.0), (24.92, 8.0)
], "VCC", "B.Cu"))
# Branch on B.Cu to U1-3 (38.09, 20.64) via a via at (35.0, 20.64)
new_elements.extend(path([
...
# 6. /HO_NET: U1-7 -> D2-1 -> R1-2
new_elements.extend(path([
    (41.91, 19.36), (45.0, 19.36), (45.0, 16.6), (48.0, 16.6)
], "/HO_NET"))
new_elements.append(via(44.5, 19.36, "/HO_NET"))
new_elements.extend(path([
    (44.5, 19.36), (44.5, 23.0), (33.92, 23.0), (33.92, 15.0)
], "/HO_NET", "B.Cu"))

# 7. /LO_NET: U1-5 -> D3-1 -> R2-2
new_elements.extend(path([
    (41.91, 21.91), (45.0, 21.91), (45.0, 26.6), (48.0, 26.6)
], "/LO_NET"))
new_elements.append(via(44.5, 21.91, "/LO_NET"))
new_elements.extend(path([
    (44.5, 21.91), (44.5, 23.0), (33.92, 23.0), (33.92, 25.0)
], "/LO_NET", "B.Cu"))
...
new_elements.extend(path([
    (60.54, 15.0), (60.54, 20.0), (58.0, 20.0), (58.0, 25.0)
], "/MID"))
new_elements.extend(path([
    (60.54, 15.0), (67.54, 15.0)
], "/MID"))
# Sense lines (0.5mm)
new_elements.extend(path([
    (58.0, 20.0), (41.91, 20.0), (41.91, 20.64)
], "/MID"))
new_elements.extend(path([
    (41.91, 20.64), (31.6, 20.64), (31.6, 15.0)
], "/MID"))
...
# 12. /RF_OUT: L2-1 -> C6-1 -> J2-1
new_elements.extend(path([
    (69.46, 15.0), (70.4, 22.0), (72.0, 22.0), (72.0, 42.0)
], "/RF_OUT"))
...
print("=== PCB ROUTING RE-BUILT SUCCESSFULLY ===")
